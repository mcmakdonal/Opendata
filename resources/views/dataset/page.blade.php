                    ...
                    <div class="col-md-12">
                        @foreach($content as $k => $v)
                            <div class="row">
                               ...
                               <iframe id="twitter-widget-0" scrolling="no" frameborder="0" allowtransparency="true" class="twitter-share-button twitter-share-button-rendered twitter-tweet-button" style="position: static; visibility: visible; width: 60px; height: 20px;float:right;margin-left:5px" title="Twitter Tweet Button" src="https://platform.twitter.com/widgets/tweet_button.4ddf50b7ac5c5f06f6679f003b742641.en.html#dnt=false&amp;lang=en&amp;original_referer=https%3A%2F%2Fdeveloper.twitter.com%2Fen%2Fdocs%2Ftwitter-for-websites%2Ftweet-button%2Foverview.html&amp;related=twitterapi%2Ctwitter&amp;size=m&amp;time=1538465345598&amp;type=share&amp;url=http%3A%2F%2Fopendata.mc%2Fdataset%2Fpage%2F{{ $v->dts_url }}" data-url="http%3A%2F%2Fopendata.mc%2Fdataset%2Fpage%2F{{ $v->dts_url }}"></iframe>
                                <iframe style="float:right" src="https://www.facebook.com/plugins/share_button.php?href=https%3A%2F%2Fgoo.gl%2FHDWS3H&layout=button_count&size=small&mobile_iframe=true&appId=1740813262901995&width=64&height=20" width="64" height="20" style="border:none;overflow:hidden" scrolling="no" frameborder="0" allowTransparency="true" allow="encrypted-media"></iframe>
                                   <div>
                                        <h3> <img src="{{ url('backend/assets/img/icon_items_pink.png') }}"> {{ $v->dts_title }} <span class="star"> {{ $v->dts_res_point }} </span> </h3>
                                        <p>{{ $v->dts_description }}</p>
                                        <p>หน่วยงาน : {{ $v->ogz_title }} - ลิขสิทธิ์ : {{ $v->license }} </p>
                                        <p>หมวดหมู่ : {{ $v->cat_title }} - วันที่ปรับปรุงข้อมูล : {{ \AppHelper::instance()->DateThai($v->update_date) }} </p>
                                        <p class="tags">{{ $v->dts_tag }}</p>
                                        <p>Metadata : <a href="/get_matadata/{{ $v->dts_id }}"  target="_blank" >ดูข้อมูล</a><a href="/get_matadata/{{ $v->dts_id }}" style="margin-left:15px"  target="_blank" download>downlaod</a></p>
                                    </div>
                                </div>
                                    ...
                                    <div class="list-group">
                                        ...
                                        <div class="list-group-item list-group-item-action">
                                            <div class="row">
                                                <div class="col-md-8">
                                                @php
                                                    $file_format = "backend/assets/img/".$v->file_format.".png";
                                                @endphp
